...
- `fdr`: a matrix of FDR-adjusted p-values for the outlier test run on each sample for each transcript in the observed data.
...
- `num.null`: the number of transcripts to generate when simulating from null distributions; default is one million.
...
Unadjusted p-values for the outlier test on each sample in each transcript are contained in the `p.values` entry of the object returned by `detect.outliers()`; FDR-adjusted p-values are contained in the `fdr` entry. Both are matrices with the same number of rows and columns as the input data, and they retain the same row and column names.

```{r examine-p-values}
outlier.test.run.1$p.values[1:6, 8:14];
outlier.test.run.1$fdr[1:6, 8:14];
```

Most of the elements of `p.values` and `fdr` will be `NA`. As explained in [Overview], testing for outliers for a given transcript stops after the first p-value is found that exceeds `p.value.threshold`. Because testing works from the most to least outlying sample, each p-value is guaranteed to be larger than the previous one. Thus, we can be sure that the missing p-values would also exceed our threshold. To ensure that the outlier test is run on each sample in each transcript, we could set the parameter `p.value.threshold` to `1`. However, this would dramatically increase the runtime of `detect.outliers()` and may not be practical in most cases.

The entries `num.outliers.unadjusted` and `num.outliers.adjusted` give transcript-specific counts of the number of outliers before and after FDR adjustment of the outlier test p-values. For `num.outliers.unadjusted`, this corresponds to the number of p-values below `p.value.threshold`; for `num.outliers.adjusted`, this corresponds to the number of FDR-adjusted p-values below `fdr.threshold`. (Note that `fdr.threshold` only impacts the reported count of outliers; it has no effect on the FDR adjustment itself.)

```{r transcript-level-outlier-counts}
head(outlier.test.run.1$num.outliers.unadjusted);
head(outlier.test.run.1$num.outliers.adjusted);
```

We can get a quick view of the distribution of the number of outliers across transcripts using `table()`:

```{r table}
table(outlier.test.run.1$num.outliers.unadjusted);
table(outlier.test.run.1$num.outliers.adjusted);
```
...
Notice that the number of rows in each data frame decreases. This again reflects the fact that for a given transcript, testing for outliers stops once a sample has been found not to be an outlier. A transcript will appear in as many rounds as it has outliers (plus one).
...
outlier.test.run.2$p.values[1:6, 8:14];
outlier.test.run.2$fdr[1:6, 8:14];

# Check the distribution of number of outliers detected.
table(outlier.test.run.2$num.outliers.unadjusted);
table(outlier.test.run.2$num.outliers.adjusted);
```
