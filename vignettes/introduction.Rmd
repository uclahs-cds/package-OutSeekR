...
- `initial.screen.method`: the statistical criterion for initial feature selection.
...
```{r examine-p-values}
outlier.test.run.1$p.values;
outlier.test.run.1$fdr;
```

As explained in [Overview], testing for outliers for a given transcript stops after the p-values or FDRs from each rouns is found that exceeds the specified threshold. 
...
outlier.test.run.2$p.values;
outlier.test.run.2$fdr;
